...
class MySQLPDOWraper
{
	...
	const RESULT_NUM   = PDO::FETCH_ASSOC;

	function __construct($dbh = NULL, $opt = array())
	{
		$opt = array_merge($this->defaults,$opt);

		$this->emode  = $opt['errmode'];
		$this->exname = $opt['exception'];
		if ($dbh == NULL) $this->dbh = connectDB_PDO();
	}
	...
	public function trans($QueryArr=array()){
		$this->begin();
		foreach ($QueryArr as $qr) {
			$res = $this->rawQuery($this->prepareQuery($qr));
			if (!$res){
				$this->rollback();
				return false;
			}
	 ...
	 */
	public function affectedRows()  //это
	{
	...
	public function execOne(){  //это
		$query = $this->prepareQuery(func_get_args());
		return $this->dbh->exec($query)==1;
	}
	/**
	...
	public function getOne()
	{
		...
		{
			$row = $res->fetch(self::RESULT_ASSOC);
			if (is_array($row)) {
				return reset($row);
			}
			$this->free($res);
	...
	public function getRow()
	{
		$query = $this->prepareQuery(func_get_args());
		if ($res = $this->rawQuery($query)) {
			$ret = $res->fetch(self::RESULT_ASSOC);
			$this->free($res);
			return $ret;
		}
		return FALSE;
	...
	public function getCol()
	{
		...
		if ( $res = $this->rawQuery($query) )
		{
			while($row = $res->fetch(self::RESULT_ASSOC))
			{
				$ret[] = reset($row);
			}
			$this->free($res);
	...
	public function getAll()
	{
		...
		if ( $res = $this->rawQuery($query) )
		{
			$ret = $res->fetchAll(self::RESULT_ASSOC);
			$this->free($res);
		}
		return $ret;
	}
	 ...
	 */
	public function getInd() //индексация происходит по поределенному столбцу и возвращается строка
	{
		$args  = func_get_args();
		...
		if ( $res = $this->rawQuery($query) )
		{
			$rows = $res->fetchAll(self::RESULT_ASSOC);
			foreach ($rows as &$r)
				$ret[$r[$index]] = $r;
			$this->free($res);
		}
	 ...
	 */
	public function getIndCol() //индексация происходит только одной колонке и получаю сразу значение
	{
		$args  = func_get_args();
		$index = array_shift($args);
		$query = $this->prepareQuery($args);
	 ...
	 */
	public function whiteList($input,$allowed,$default=FALSE) //это
	{
	 ...
	 */
	public function filterArray($input,$allowed) //это
	{
	...
	public function createIN($data)
	{
		if (!is_array($data))
		{
			$this->error("Value for IN (?a) placeholder should be array");//дополнительной выборки оператор in
			return;
		}
		if (!$data)
		{
	...
	private function createSET($data)
	{
		if (!is_array($data))
		{
			$this->error("SET (?u) placeholder expects array, ".gettype($data)." given"); //для адейта, принимает ключ значение
			return;
		}
		if (!$data)
		{
